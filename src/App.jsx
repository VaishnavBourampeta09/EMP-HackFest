  ...
  useEffect(() => {
    const id = setInterval(() => {
      ...
      if (sim.stopped) {
        ...
        setState({ simulation: { ...sim, minutesStopped, lastTickAt: Date.now() } });
      ...
      setState({ simulation: { ...sim, index: nextIndex, minutesStopped: 0, lastTickAt: Date.now() } });
    ...
    () => [
      {
        ...
        onClick: () =>
          setState({
            simulation: { ...getState().simulation, stopped: !simulation.stopped, minutesStopped: 0 }
          })
      },
      {
        ...
        onClick: () => {
          ...
          setState({
            simulation: { ...sim, offRoute: goingOffRoute, index, stopped: false, minutesStopped: 0 }
          });
        }
      },
      ...
      <header className="app-bar">
        <div className="brand">
          ...
          <div>
            ...
            <span className="tagline">From passive tracking to proactive protection</span>
          </div>
        </div>
        <div className="mode-toggle">
          ...
          >
            Parent
            {mode !== 'parent' && checkin && checkin.status === 'expired' && <span className="dot" />}
          </button>
        </div>
      </header>
      ...
      <footer className="demo-bar">
        ...
        <div className="demo-buttons">
          {controls.map((control) => (
            <button key={control.label} type="button" disabled={control.disabled} onClick={control.onClick}>
              {control.label}
            </button>
          ))}
        </div>
